...
class Cashier extends Employee {
    ...
    public void printCashierDetails() {
        printEmployeeDetails(); // Calling method from superclass
    }
    ...
    public int Open(Game games[], Register register, int cookie_total, Game damaged_games[]){
        ...
        for (int i = 0; i < num_customers; i++){ //loop through customers
            ...
            int cookie_monster = new Random().nextInt(100);
            if (cookie_monster == 1 && cookie_total != 0) {
                cookie_total = 0; 
            ...
            for(int j = 0; j < games.length; j++){ //loop through games
                purchase_chance += 22 - (2*games[j].getShelfPosition());
...
class GameStore{
    ...
    public GameStore(Game games[],Employee employees[], Baker baker, Announcer announcer){ //paramaterized constructor
    ...
    public void run_day(){
        ...
        num_cookies = cashier.Open(this.games_, this.register_, num_cookies, this.damaged_);
        num_cookies_sold -= num_cookies;
        ...
        if (this.day == 1) {
            ...
            if (robbed == 0) { //The store was robbed; all money, cookies and games are removed. 
                ...
                this.announcer_("The store was robbed after day 1. All money, cookies and games are gone."); 
                this.day += 2; 
                this.announcer_("The store was closed for day 2 as insurance replaces the stock. It is now day 3."); 
            }
        }
        announcer_.leave();
    }
...
class problem1 {
    public static void main(String[] args) {
        ...
        GameStore simulation = new GameStore(games, employees, baker, announcer);
        for(int i = 0; i < 30; i++){
            simulation.run_day();
            System.out.println("\n");
        }
